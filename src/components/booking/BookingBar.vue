...
const emit = defineEmits<{
  (e: 'reserve', data: ReserveData): void
}>()
  ...
  >
      ...
      >
        <div class="flex items-center justify-between mb-[0.8rem]">
          ...
          <div class="w-8 flex justify-start">
            <button
              v-if="monthIndex === 0 && canGoBack"
              type="button"
              class="border-0 bg-transparent text-[#8d98ac] hover:text-brand text-[1.45rem] leading-none cursor-pointer transition-colors"
          ...
          <h4 class="m-0 text-[#1f2a39] text-base font-bold capitalize text-center flex-1">
            {{ month.title }}
          </h4>
        ...
        <div class="grid grid-cols-7 gap-[0.08rem]">
          <button
            ...
            type="button"
            :class="[
              'min-h-[2.45rem] border-0 rounded-[0.18rem] font-[inherit] cursor-pointer transition-all duration-150',
              cell.disabled
                ? 'text-[#d2d8e3] cursor-not-allowed'
                : 'text-[#243143] hover:bg-brand/10 hover:text-brand-dark',
              !cell.inCurrentMonth && 'text-[#d2d8e3]',
              isDateInSelectedRangeExclusive(cell.date) && 'bg-[#dcefd7] text-[#274026]',
              (isStartDate(cell.date) || isEndDate(cell.date)) &&
                'bg-[#3b7f2a] text-white hover:bg-[#3b7f2a] hover:text-white',
            ]"
    ...
    >
      ...
      >
        <button
          type="button"
          class="grid content-center gap-[0.16rem] min-h-18 px-[0.92rem] py-[0.72rem] border-0 border-r border-[#c7cfdb] text-[#617463] bg-white hover:bg-brand-light/50 w-full cursor-pointer text-left transition-colors duration-200"
        ...
        <button
          type="button"
          class="grid content-center gap-[0.16rem] min-h-18 px-[0.92rem] py-[0.72rem] border-0 text-[#617463] bg-white hover:bg-brand-light/50 w-full cursor-pointer text-left transition-colors duration-200"
      ...
      <!-- Guests Picker -->
      <div
        class="relative grid grid-rows-[auto_1fr] content-center gap-[0.16rem] min-h-18 max-sm:min-h-17 px-[0.92rem] py-[0.72rem] border border-[#c7cfdb] text-[#617463] bg-white hover:bg-brand-light/30 max-md:col-start-1 max-md:col-end-2 max-sm:col-span-1 transition-colors duration-200"
      >
        <span class="text-[0.68rem] uppercase tracking-[0.11em] text-[#7b8b7d] whitespace-nowrap"
          >Huéspedes</span
        >
        ...
        >
          ...
          <span class="text-[#94a0b2] text-[1.2rem] leading-none transition-transform duration-200"
            >⌄</span
          >
        </button>
        ...
        >
          ...
          <div class="flex items-center justify-between gap-4 pt-[0.35rem] pb-[0.9rem]">
            ...
            >
              <button
                type="button"
                class="w-[2.2rem] h-[2.2rem] border-0 text-[#9aa4b7] bg-white text-[1.15rem] cursor-pointer hover:bg-gray-100 hover:text-brand transition-colors"
          ...
          <button
            type="button"
            class="w-full min-h-11 mt-3 border border-brand rounded-[0.18rem] text-brand bg-white hover:bg-brand hover:text-white text-base cursor-pointer transition-all duration-200 font-semibold"
            @click="acceptGuests"
          >
            Aceptar
          </button>
      ...
      <label
        class="flex flex-col justify-center items-center min-h-18 max-md:min-h-19 max-sm:min-h-17 px-[1.2rem] py-[0.82rem] border border-[#c7cfdb] focus-within:border-brand text-[#617463] bg-white text-center max-md:col-start-2 max-md:col-end-3 max-sm:col-span-1 transition-colors duration-200"
      >
        <input
          type="text"
          v-model="promoCode"
          ...
          class="block w-full border-0 outline-none bg-transparent text-[#183b11] text-[1.08rem] font-normal text-center placeholder:text-[#8b95a9]"
        />
      </label>
      ...
      <button
        type="button"
        @click="handleReserve"
        class="min-w-42 max-md:col-span-2 max-md:min-h-[3.4rem] max-sm:col-span-1 max-sm:w-full border-0 rounded-[0.18rem] px-5 text-white font-extrabold text-[0.94rem] tracking-wider bg-[linear-gradient(135deg,#34852d,#3f8c27)] cursor-pointer hover:brightness-110 active:scale-[0.98] transition-all duration-200 shadow-md hover:shadow-lg"
      >
        RESERVAR
      </button>
    </div>
